...
SipMessage::SipMessage(const char *messageBytes, int byteCount)
    : HttpMessage(messageBytes, byteCount), mpSecurity(NULL), mpEventData(NULL),
      mbFromThisSide(true) {
...
SipMessage::SipMessage(OsSocket *inSocket, int bufferSize)
    : HttpMessage(inSocket, bufferSize), mpSecurity(NULL), mpEventData(NULL), mbFromThisSide(true) {
#ifdef TRACK_LIFE
    osPrintf("Created SipMessage @ address:%X\n", this);
#endif
    mbUseShortNames = false;
...
// Copy constructor
SipMessage::SipMessage(const SipMessage &rSipMessage) : HttpMessage(rSipMessage), mpSecurity(NULL) {
#ifdef TRACK_LIFE
    osPrintf("Created SipMessage @ address:%X\n", this);
#endif
    replaceShortFieldNames();
...
// Assignment operator
SipMessage &SipMessage::operator=(const SipMessage &rSipMessage) {
...
UtlBoolean SipMessage::getShortName(const char *longFieldName, UtlString *shortFieldName) {
...
UtlBoolean SipMessage::getLongName(const char *shortFieldName, UtlString *longFieldName) {
    UtlBoolean nameFound = FALSE;

    // Short names are currently only 1 character long
    // If the short name is exactly 1 character long
    if (shortFieldName && shortFieldName[0] && shortFieldName[1] == '\0') {
...
void SipMessage::replaceShortFieldNames() {
    ...
    for (position = 0; (nvPair = static_cast<NameValuePair *>(mNameValues.at(position)));
         position++) {
...
void SipMessage::setReinviteData(SipMessage *invite,
    ...
    if (farEndContact) lastResponseContact.append(farEndContact);
    ...
    if (!inviteFromThisSide && lastResponseContact.isNull()) {
        ...
        // final responses from the other side yet ...check the otherside's invite request and get
        // the contact field from the request
...
void SipMessage::setInviteData(const char *fromField,
    ...
    if (sessionReinviteTimer > 0) setSessionExpires(sessionReinviteTimer);
...
void SipMessage::addSdpBody(int nRtpContacts,
    ...
    if (numRtpCodecs > 0) {
        ...
        sdpBody->setStandardHeaderFields("call", NULL, NULL,
...
const SdpBody *SipMessage::getSdpBody(SIPXTACK_SECURITY_ATTRIBUTES *const pSecurity,
    ...
    if (pEventData) { mpEventData = (void *)pEventData; }
    ...
    // If we have a private key and this is a S/MIME body
        ...
        if (genericBody) {
            ...
            if (!smimeBody->isDecrypted()) {
                if (pSecurity) { mpSecurity = pSecurity; }
    ...
    // if there is an SDP part
        ...
        if (multipartBody && multipartBody->isMultipart()) {
            ...
            while ((bodyPart = multipartBody->getMultipart(partIndex))) {
                ...
                else if (strcmp(bodyPart->getContentType(), smimeType) == 0 && pSecurity) {
...
void SipMessage::setRequestData(const char *method,
                                const char *uri,
    ...
    if (contactUrl && *contactUrl) { setContactField(contactUrl); }
}
...
void SipMessage::setResponseData(int statusCode,
    ...
    // Add the CSeq field
    if (sequenceNumber >= 0) { setCSeqField(sequenceNumber, sequenceMethod); }

    // Local Contact
    if (localContact) { setContactField(localContact); }
}
...
void SipMessage::setInviteBusyData(const char *fromField,
                                   const char *toField,
                                   const char *callId,
                                   int sequenceNumber) {
    setResponseData(SIP_BUSY_CODE,
                    SIP_BUSY_TEXT,
                    fromField,
                    toField,
                    callId,
...
void SipMessage::setBadTransactionData(const SipMessage *inviteRequest) {
    setResponseData(inviteRequest, SIP_BAD_TRANSACTION_CODE, SIP_BAD_TRANSACTION_TEXT);
}

void SipMessage::setLoopDetectedData(const SipMessage *inviteRequest) {
    setResponseData(inviteRequest, SIP_LOOP_DETECTED_CODE, SIP_LOOP_DETECTED_TEXT);
}
...
void SipMessage::setInviteErrorData(const SipMessage *inviteRequest,
                                    int errorCode,
    ...
    setResponseData(
        errorCode, errorText, fromField, toField, callId, sequenceNum, SIP_INVITE_METHOD);
...
void SipMessage::setRequestPendingData(const SipMessage *inviteRequest) {
    ...
    setResponseData(SIP_REQUEST_PENDING_CODE,
                    SIP_REQUEST_PENDING_TEXT,
                    fromField,
                    toField,
                    callId,
                    sequenceNum,
...
void SipMessage::setForwardResponseData(const SipMessage *request, const char *forwardAddress) {
...
void SipMessage::setInviteBadCodecs(const SipMessage *inviteRequest, SipUserAgent *ua) {
    char warningCodeString[MAXIMUM_INTEGER_STRING_LENGTH + 1];
    UtlString warningField;

    setResponseData(
        inviteRequest, SIP_REQUEST_NOT_ACCEPTABLE_HERE_CODE, SIP_REQUEST_NOT_ACCEPTABLE_HERE_TEXT);
...
void SipMessage::setRequestBadMethod(const SipMessage *request, const char *allowedMethods) {
...
void SipMessage::setRequestUnimplemented(const SipMessage *request) {
    setResponseData(request, SIP_UNIMPLEMENTED_METHOD_CODE, SIP_UNIMPLEMENTED_METHOD_TEXT);
}
...
void SipMessage::setRequestBadUrlType(const SipMessage *request) {
    setResponseData(request, SIP_UNSUPPORTED_URI_SCHEME_CODE, SIP_UNSUPPORTED_URI_SCHEME_TEXT);
}
...
void SipMessage::setInviteOkData(const SipMessage *inviteRequest,
    ...
    if (mpSecurity && !inviteSdp) {
        ...
        if (!inviteRequest->getSessionExpires(&inviteSessionExpires, &refresher)
            || (maxSessionExpiresSeconds > 0 && inviteSessionExpires > maxSessionExpiresSeconds)) {
...
int SipMessage::setInviteOkRoutes(const SipMessage &inviteRequest) {
    ...
    while (inviteRequest.getRecordRouteField(recordRouteIndex, &recordRouteField)) {
...
void SipMessage::setOkResponseData(const SipMessage *request, const char *localContact) {
...
void SipMessage::setNotifyData(SipMessage *subscribeRequest,
    ...
    if (eventField && *eventField) {
        ...
        if (!subscribeEventField.isNull()) { setEventField(subscribeEventField.data()); }
    }
    ...
    setRequestData(SIP_NOTIFY_METHOD,
                   uri.data(),
                   toField.data(),
                   fromField.data(),
                   callId,
                   localCSequenceNumber);
}
...
void SipMessage::setSubscribeData(const char *uri,
    ...
    setRequestData(
        SIP_SUBSCRIBE_METHOD, uri, fromUrl.toString(), toUrl.toString(), callId, cseq, contact);
    ...
    if (acceptField && *acceptField) { setHeaderValue(SIP_ACCEPT_FIELD, acceptField, 0); }

    // Set the route, if any.
    if (routeField && *routeField) { setRouteField(routeField); }
...
void SipMessage::setEnrollmentData(const char *uri,
                                   ...
                                   int expiresInSeconds) {
    setRequestData(SIP_SUBSCRIBE_METHOD, uri, fromField, toField, callId, CSeq, contactUrl);
...
void SipMessage::setMessageSummaryData(UtlString &msgSummaryData,
...
void SipMessage::setMWIData(const char *method,
                            ...
                            int CSeq,
                            UtlString bodyString) {
    setRequestData(method, uri, fromField, toField, callId, CSeq, contactUrl);
    // Set the allow field
    ...
    HttpBody *httpBody =
        new HttpBody(bodyString.data(), bodyString.length(), CONTENT_TYPE_SIMPLE_MESSAGE_SUMMARY);
...
void SipMessage::setRequestUnauthorized(const SipMessage *request,
    ...
    if (authEntity == SERVER) {
        setResponseData(request, HTTP_UNAUTHORIZED_CODE, HTTP_UNAUTHORIZED_TEXT);
    } else {
        setResponseData(request, HTTP_PROXY_UNAUTHORIZED_CODE, HTTP_PROXY_UNAUTHORIZED_TEXT);
    }

    setAuthenticationData(authenticationScheme,
                          authenticationRealm,
                          authenticationNonce,
                          authenticationOpaque,
                          NULL,
                          authEntity);
}
...
UtlBoolean SipMessage::verifyMd5Authorization(const char *userId,
    ...
    if (isResponse()) {
        ...
        // which the validation will fail).
            ...
            OsSysLog::add(FAC_SIP,
                          PRI_DEBUG,
                          "SipMessage::verifyMd5Authorization using request URI: %s instead of "
                          "Auth header uri parameter for digest\n",
                          uriString.data());
        }
    ...
    isAllowed = HttpMessage::verifyMd5Authorization(
        userId, password, nonce, realm, method.data(), uriString.data(), authEntity);
    return isAllowed;
}
...
void SipMessage::setAckData(const char *uri,
                            ...
                            const char *callId,
                            int sequenceNumber) {
    setRequestData(SIP_ACK_METHOD, uri, fromField, toField, callId, sequenceNumber);
}
...
void SipMessage::setAckData(const SipMessage *inviteResponse,
    ...
    if (responseCode >= SIP_OK_CODE && responseCode < SIP_MULTI_CHOICE_CODE) {
        ...
        if (inviteResponse->buildRouteField(&routeField)) { setRouteField(routeField.data()); }
    ...
    if (sessionTimerExpires > 0) setSessionExpires(sessionTimerExpires);
}
...
void SipMessage::setByeData(const char *uri,
                            ...
                            const char *localContact,
                            int sequenceNumber) {
    setRequestData(SIP_BYE_METHOD, uri, fromField, toField, callId, sequenceNumber, localContact);
}
...
void SipMessage::setByeData(const SipMessage *inviteRequest,
    ...
    if (remoteContact) remoteContactString.append(remoteContact);
    ...
    if (!remoteContactString.isNull()) { uri.append(remoteContactString); }
...
void SipMessage::setReferData(const SipMessage *inviteRequest,
    ...
    if (referToCallee) {
        setRequestData(SIP_REFER_METHOD,
                       uri.data(),
                       fromField.data(),
                       toField.data(),
                       callId,
...
void SipMessage::setOptionsData(const SipMessage *inviteRequest,
    ...
    if (remoteContact && *remoteContact) { uri.append(remoteContact); }
    ...
    UtlString referredByField;
    if (optionsToCallee) {
        setRequestData(SIP_OPTIONS_METHOD,
                       uri.data(),
                       fromField.data(),
                       toField.data(),
                       callId,
...
void SipMessage::setCancelData(const char *fromField,
                               const char *toField,
                               const char *callId,
                               int sequenceNumber,
                               const char *localContact) {
    setRequestData(
        SIP_CANCEL_METHOD, toField, fromField, toField, callId, sequenceNumber, localContact);
}

void SipMessage::setCancelData(const SipMessage *inviteRequest, const char *localContact) {
    UtlString uri;
    UtlString fromField;
    UtlString toField;
    UtlString callId;
    ...
    setRequestData(SIP_CANCEL_METHOD, uri, fromField, toField, callId, sequenceNum, localContact);
}
...
void SipMessage::setPublishData(const char *uri,
                                ...
                                int expiresInSeconds,
                                const char *contact) {
    setRequestData(SIP_PUBLISH_METHOD, uri, fromField, toField, callId, cseq, contact);
    ...
    if (sipIfMatchField && *sipIfMatchField) { setSipIfMatchField(sipIfMatchField); }
    ...
    for (header = 0; requestUri.getHeaderParameter(header, hdrName, hdrValue); header++) {
        ...
        if (isUrlHeaderAllowed(hdrName.data())) {
            if (0 == hdrName.compareTo(SIP_FROM_FIELD, UtlString::ignoreCase)) {
                ...
                int route;
                UtlString thisRoute;
                for (route = 0; UtlNameValueTokenizer::getSubField(
                         hdrValue.data(), route, SIP_MULTIFIELD_SEPARATOR, &thisRoute);
                    ...
                    if (!routeParams.isNull()) { routeParams.append(SIP_MULTIFIELD_SEPARATOR); }
...
void SipMessage::addViaField(const char *viaField, UtlBoolean afterOtherVias) {
    ...
    if (fieldIndex == UTL_NOT_FOUND) {
        ...
        while (nv = (NameValuePair *)iterator()) { osPrintf("\tName: %s\n", nv->data()); }
#endif
    }
...
void SipMessage::setLastViaTag(const char *tagValue, const char *tagName) {
    UtlString lastVia;
    ...
    while ((nvPair = (NameValuePair *)iterator())) {
        ...
        if (newVia.length()) newVia.append(";");
        ...
        // if the value we are looking for is found, then we are going to replace the value with
        // this value
...
void SipMessage::setReceivedViaParams(const UtlString &fromIpAddress, int fromPort) {
    ...
    if (!portIsValid(lastPort)) { tempLastPort = 5060; }
...
void SipMessage::setCallIdField(const char *callId) { setHeaderValue(SIP_CALLID_FIELD, callId); }
...
void SipMessage::buildSipUrl(UtlString *url,
                             const char *address,
                             int port,
                             const char *protocol,
                             const char *user,
    ...
    if (userLabel && strlen(userLabel)) { url->append(">"); }
...
void SipMessage::setFromField(const char *url) {
    ...
    parseAddressFromUri(url, &address, &port, &protocol, &user, &userLabel);
    buildSipUrl(&value, address.data(), port, protocol.data(), user.data(), userLabel.data());

    // If the field exists change it, if does not exist create it.
    setHeaderValue(SIP_FROM_FIELD, value.data(), 0);
}

void SipMessage::setFromField(const char *address,
                              int port,
                              const char *protocol,
                              const char *user,
                              const char *userLabel) {
...
void SipMessage::setRawToField(const char *url) { setHeaderValue(SIP_TO_FIELD, url, 0); }
...
void SipMessage::setToField(const char *address,
                            int port,
                            const char *protocol,
                            const char *user,
                            const char *userLabel) {
...
void SipMessage::setWarningField(int code, const char *hostname, const char *text) {
    ...
    size_t sizeNeeded =
        3 /* warning code size */ + strlen(hostname) + strlen(text) + 3 /* blanks & null */;
    ...
    if (allocated >= sizeNeeded) {
        ...
        OsSysLog::add(FAC_SIP,
                      PRI_WARNING,
                      "SipMessage::setWarningField value too large (max %" PRIuPTR
                      ") host '%s' text '%s'",
                      allocated,
                      hostname,
                      text);
...
UtlBoolean SipMessage::getMaxForwards(int &maxForwards) const {
    ...
    if (value) { maxForwards = atoi(value); }
    return (value != NULL);
}
...
void SipMessage::decrementMaxForwards() {
    int maxForwards;
    if (!getMaxForwards(maxForwards)) { maxForwards = SIP_DEFAULT_MAX_FORWARDS; }
    maxForwards--;
...
UtlBoolean SipMessage::getPAssertedIdentityField(UtlString &identity, int index) const {
    UtlBoolean fieldExists = getFieldSubfield(SIP_P_ASSERTED_IDENTITY_FIELD, index, &identity);
...
UtlBoolean SipMessage::parseParameterFromUri(const char *uri,
    ...
    if (parameterStart >= 0) {
        ...
        UtlNameValueTokenizer::getSubField(uriString.data(), 0, " \t;>", parameterValue);
    }
...
void SipMessage::parseAddressFromUri(const char *uri,
    ...
    if (address) { parsedUri.getHostAddress(*address); }
    if (port) { *port = parsedUri.getHostPort(); }
    if (protocol) { parsedUri.getUrlParameter("transport", *protocol); }
    if (user) { parsedUri.getUserId(*user); }
    if (userLabel) { parsedUri.getDisplayName(*userLabel); }
    if (tag) { parsedUri.getFieldParameter("tag", *tag); }
}

void SipMessage::setUriParameter(UtlString *uri,
                                 const char *parameterName,
                                 const char *parameterValue) {
    UtlString parameterString(parameterName);

    // only append the '=' if not null and has a length
    if (parameterValue && *parameterValue != '\0') parameterString.append('=');
    ...
    if (tagIndex >= 0) {
        ...
        int tagEnd = tagSpace;
        if (tagTab >= tagIndex && (tagTab < tagEnd || tagEnd < tagIndex)) { tagEnd = tagTab; }
        if (tagSemi >= tagIndex && (tagSemi < tagEnd || tagEnd < tagIndex)) { tagEnd = tagSemi; }
        ...
        if (parameterValue && *parameterValue != '\0') uri->insert(tagIndex, parameterValue);
    }
    ...
    // Tag does not exist append it
        ...
        if (parameterValue && *parameterValue != '\0') uri->append(parameterValue);
    }
}
...
void SipMessage::getUri(UtlString *address, int *port, UtlString *protocol, UtlString *user) const {
    ...
    if (!uriField.isNull()) {
        ...
        if (address) { uriUrl.getHostAddress(*address); }
        if (protocol) { uriUrl.getUrlParameter("transport", *protocol); }
        if (port) { *port = uriUrl.getHostPort(); }

        if (user) { uriUrl.getUserId(*user); }
    }
    // parseAddressFromUri(uriField.data(), address, port, protocol, user);
}

void SipMessage::getFromAddress(UtlString *address,
                                int *port,
                                UtlString *protocol,
                                UtlString *user,
                                UtlString *userLabel,
    ...
    parseAddressFromUri(uri.data(), address, port, protocol, user, userLabel, tag);
}

void SipMessage::getToAddress(UtlString *address,
                              int *port,
                              UtlString *protocol,
                              UtlString *user,
                              UtlString *userLabel,
...
UtlBoolean
    SipMessage::getResponseSendAddress(UtlString &address, int &port, UtlString &protocol) const {
    int receivedPort;
    UtlBoolean receivedSet;
    UtlBoolean maddrSet;
    UtlBoolean receivedPortSet;
    ...
    getLastVia(
        &address, &port, &protocol, &receivedPort, &receivedSet, &maddrSet, &receivedPortSet);
    ...
    if (receivedPortSet && portIsValid(receivedPort)) {
        OsSysLog::add(FAC_SIP,
                      PRI_DEBUG,
                      "SipMessage::getResponseSendAddress response to receivedPort %s:%d not %d\n",
                      address.data(),
                      receivedPort,
                      port);
        port = receivedPort;
...
UtlBoolean SipMessage::removeLastVia() {
    ...
    // add updated line
    if (!NewViaHeader.isNull()) { addViaField(NewViaHeader); }
    return (fieldFound);
}
...
UtlBoolean SipMessage::getViaField(UtlString *viaField, int index) const {
    ...
    viaField->remove(0);
    if (value) { viaField->append(value); }
    return (value != NULL);
}
...
void SipMessage::getLastVia(UtlString *address,
    ...
    *port = PORT_NONE;
    ...
    if (port) { *port = 0; }
    if (address) { address->remove(0); }
    if (protocol) { protocol->remove(0); }
    if (receivedSet) { *receivedSet = FALSE; }
    if (maddrSet) { *maddrSet = FALSE; }
    if (receivedPortSet) { *receivedPortSet = FALSE; }
    ...
    if (getFieldSubfield(SIP_VIA_FIELD, 0, &Via)) {
        UtlNameValueTokenizer::getSubField(Via, 0, SIP_SUBFIELD_SEPARATORS, &sipProtocol);
        UtlNameValueTokenizer::getSubField(Via, 1, SIP_SUBFIELD_SEPARATORS, &url);
        ...
        if (index >= 0) { sipProtocol.remove(0, index + 1); }
        if (protocol) { protocol->append(sipProtocol.data()); }

        Url viaParam(url, TRUE);
        if (address) { viaParam.getHostAddress(*address); }
        if (port) { *port = viaParam.getHostPort(); }
        ...
        if (receivedPort && !receivedPortString.isNull()) {
            *receivedPort = atoi(receivedPortString.data());
        } else if (receivedPort) {
            *receivedPort = PORT_NONE;
        }

        if (receivedSet) { *receivedSet = receivedFound; }
        if (maddrSet) { *maddrSet = maddrFound; }
        if (receivedPortSet) { *receivedPortSet = receivedPortFound; }
    }
}

UtlBoolean SipMessage::getViaTag(const char *viaField, const char *tagName, UtlString &tagValue) {
    ...
    if (value) {
       ...
       *sequenceNum = atoi(sequenceNumString.data());
       ...
       UtlNameValueTokenizer::getSubField(value, 1,
        ...
        if (sequenceMethod) { sequenceMethod->remove(0); }
    }
...
#ifdef TEST_PRINT
                        osPrintf(
                            "SipMessage::getContactEntry found contact[%d] starting: %d ending: %d "
                            "\"%s\"\n",
                            addressIndex,
                            addressStart,
                            charIndex,
                            uriAndParameters->data());
#endif
                        currentEntryValue++;
                        contactFound = TRUE;
                        break;
...
UtlBoolean SipMessage::getContactAddress(int addressIndex,
    ...
    if (foundUri)
        parseAddressFromUri(uri.data(), contactAddress, contactPort, protocol, user, userLabel);

    return (foundUri);
}

UtlBoolean SipMessage::getRequireExtension(int extensionIndex, UtlString *extension) const {
    return (getFieldSubfield(SIP_REQUIRE_FIELD, extensionIndex, extension));
}

UtlBoolean SipMessage::getProxyRequireExtension(int extensionIndex, UtlString *extension) const {
...
UtlBoolean
    SipMessage::getEventField(UtlString *eventType,
    ...
    if (NULL != eventId) { eventId->remove(0); }
    ...
    if (gotHeader) {
        ...
        for (int param_idx = 1;
            ...
            if (paramPair.getNextPair('=', &name, &value)) {
                ...
                OsSysLog::add(
                    FAC_SIP, PRI_WARNING, "invalid event parameter '%s'", eventParam.data());
            }
        }
    }
...
UtlBoolean SipMessage::getEventField(UtlString &eventField) const {
    ...
    if (value) { eventField.append(value); }
...
UtlBoolean SipMessage::getExpiresField(int *expiresInSeconds) const {
    ...
    if (fieldValue) {
        ...
        UtlNameValueTokenizer::getSubField(fieldValue, 1, " \t:;,", &subfieldText);
...
UtlBoolean SipMessage::getRequestDispositionField(UtlString *dispositionField) const {
    ...
    if (value) { dispositionField->append(value); }

    return (value != NULL);
}

UtlBoolean SipMessage::getRequestDisposition(int tokenIndex, UtlString *dispositionToken) const {
    return (getFieldSubfield(SIP_REQUEST_DISPOSITION_FIELD, tokenIndex, dispositionToken));
}
...
void SipMessage::setRecordRouteField(const char *recordRouteField, int index) {
...
void SipMessage::addLastRouteUri(const char *routeUri) {
    if (routeUri && *routeUri) {
        ...
        while ((routeField = getHeaderValue(index, SIP_ROUTE_FIELD))) { index++; }
        ...
        if (strstr(routeUri, "<") == nullptr) { routeString.append("<"); }
...
UtlBoolean SipMessage::getLastRouteUri(UtlString &routeUri, int &lastIndex) {
    int index = 0;
...
UtlBoolean SipMessage::removeRouteUri(int index, UtlString *routeUri) {
    ...
    while (getFieldSubfield(SIP_ROUTE_FIELD, uriIndex, &aRouteUri)) {
        ...
        if (uriIndex == index) {
            *routeUri = aRouteUri;
            ...
            if (!newRouteField.isNull()) { newRouteField.append(','); }
    ...
    // uri removed.
    if (!newRouteField.isNull()) { insertHeaderField(SIP_ROUTE_FIELD, newRouteField.data()); }
...
#endif
    if (routeField && strlen(routeField)) { setHeaderValue(SIP_ROUTE_FIELD, routeField, 0); }
}
...
UtlBoolean SipMessage::buildRouteField(UtlString *routeFld) const {
    ...
    if (!isResponse()) {
        ...
        while (getRecordRouteUri(recordRouteIndex, &recordRouteUri)) {
            if (!routeField.isNull()) { routeField.append(','); }
    ...
    // If response build from recordeRoute in reverse order
        ...
        while (getRecordRouteUri(index, &recordRouteUri)) {
            ...
            if (index > 0) { routeField.insert(0, ", "); }
...
UtlBoolean
    SipMessage::getFieldSubfield(const char *fieldName, int addressIndex, UtlString *uri) const {
    UtlBoolean uriFound = FALSE;
    UtlString url;
    int fieldIndex = 0;
    int subFieldIndex = 0;
    ...
    while (value && index <= addressIndex) {
        ...
        while (!url.isNull() && index < addressIndex) {
            ...
            UtlNameValueTokenizer::getSubField(
                value, subFieldIndex, SIP_MULTIFIELD_SEPARATOR, &url);
#ifdef TEST
            osPrintf("Got field: \"%s\" subfield[%d]: %s\n", fieldName, fieldIndex, url.data());
#endif
        }
...
UtlBoolean SipMessage::getContentEncodingField(UtlString *contentEncodingField) const {
    ...
    if (value) { contentEncodingField->append(value); }
    return (value != NULL);
}
...
bool SipMessage::hasSelfHeader() const {
    ...
    if (value.isNull()) { getServerField(&value); }
    return !value.isNull();
}
...
void SipMessage::getServerField(UtlString *serverFieldValue) const {
    ...
    if (server) { serverFieldValue->append(server); }
}
...
UtlBoolean SipMessage::isInSupportedField(const char *token) const {
    ...
    while (value && !tokenFound) {
        ...
        OsSysLog::add(FAC_SIP,
                      PRI_DEBUG,
                      "Got field: \"%s\" subfield[%d]: %s\n",
                      value,
                      fieldIndex,
                      url.data());
#endif
        url.strip(UtlString::both);
        if (url.compareTo(token, UtlString::ignoreCase) == 0) { tokenFound = TRUE; }
...
void SipMessage::setAlsoField(const char *alsoField) { setHeaderValue(SIP_ALSO_FIELD, alsoField); }
...
UtlBoolean SipMessage::getReferredByUrls(UtlString *referrerUrl, UtlString *referredToUrl) const {
    if (referrerUrl) referrerUrl->remove(0);
    if (referredToUrl) referredToUrl->remove(0);
    const char *value = getHeaderValue(0, SIP_REFERRED_BY_FIELD);
    if (value) {
        // The first element is the referrer URL
        if (referrerUrl) UtlNameValueTokenizer::getSubField(value, 0, ";", referrerUrl);

        // The second element is the referred to URL
        if (referredToUrl) UtlNameValueTokenizer::getSubField(value, 1, ";", referredToUrl);
    }
    return (value != NULL);
}

UtlBoolean
    SipMessage::getReplacesData(UtlString &callId, UtlString &toTag, UtlString &fromTag) const {
    ...
    if (replacesField) {
        // Get the callId
        UtlNameValueTokenizer::getSubField(replacesField, 0, ";", &callId);
        callId.strip(UtlString::both);

        // Look through the rest of the parameters
        do {
            // Get a name value pair
            UtlNameValueTokenizer::getSubField(replacesField, parameterIndex, ";", &parameter);


            // Parse out the parameter name
            UtlNameValueTokenizer::getSubField(parameter.data(), 0, "=", &name);
            name.toLower();
            name.strip(UtlString::both);

            // Parse out the parameter value
            UtlNameValueTokenizer::getSubField(parameter.data(), 1, "=", &value);
...
void SipMessage::setReasonField(const char *reasonField) {
    if (NULL != reasonField) { setHeaderValue(SIP_REASON_FIELD, reasonField); }
}
...
void SipMessage::addDiversionField(const char *addr,
                                   const char *reasonParam,
                                   UtlBoolean afterOtherDiversions) {
    if (NULL == addr || NULL == reasonParam) { return; }
...
UtlBoolean SipMessage::getLastDiversionField(UtlString &diversionField, int &lastIndex) {
    int index = 0;
...
UtlBoolean SipMessage::isResponse() const {
    ...
    if (mFirstHeaderLine.index(SIP_PROTOCOL_VERSION) == 0) { responseType = TRUE; }
    ...
    if (message) {
        ...
        if (thisCallId.compareTo(thatCallId) == 0) {
            ...
            if (thisFrom.compareTo(thatFrom) == 0) {
                ...
                UtlString thatTag;
                getFromAddress(
                    &thisAddress, &thisPort, &thisProtocol, &thisUser, &thisUserLabel, &thisTag);
                message->getFromAddress(
                    &thatAddress, &thatPort, &thatProtocol, &thatUser, &thatUserLabel, &thatTag);

                if (thisAddress.compareTo(thatAddress) == 0
                    && (thisPort == thatPort || (thisPort == PORT_NONE && thatPort == SIP_PORT)
...
#ifdef TEST_PRINT
                    osPrintf(
                        "ERROR: From field did not match: \nAddr: (%s!=%s)\nPort: %d!=%d\nUser: "
                        "(%s!=%s)\nTag:  (%s!=%s)\n",
                        thisAddress.data(),
                        thatAddress.data(),
                        thisPort,
...
UtlBoolean SipMessage::isSameSession(Url &oldUrl, Url &newUrl) {
    ...
    if (thisAddress.compareTo(thatAddress) == 0
        && (thisPort == thatPort || (thisPort == 0 && thatPort == SIP_PORT)
            || (thisPort == SIP_PORT && thatPort == 0))
        && thisProtocol.compareTo(thatProtocol) == 0 && thisUser.compareTo(thatUser) == 0
...
#ifdef TEST_PRINT
        osPrintf(
            "SipMessage::isSameSession Url did not match: \nAddr: (%s!=%s)\nPort: %d!=%d\nUser: "
            "(%s!=%s)\nTag:  (%s!=%s)\n",
            thisAddress.data(),
            thatAddress.data(),
            thisPort,
...
UtlBoolean SipMessage::isSameTransaction(const SipMessage *message) const {
    ...
    if (isSameSession(message)) {
        getCSeqField(&thisSequenceNum, &thisSequenceMethod);
        message->getCSeqField(&thatSequenceNum, &thatSequenceMethod);
        if (thisSequenceNum == thatSequenceNum) { isPair = TRUE; }
    }
    return (isPair);
}
...
UtlBoolean SipMessage::isRequireExtensionSet(const char *extension) const {
    ...
    while (getRequireExtension(extensionIndex, &extensionString)) {
        ...
        if (extensionString.compareTo(extension) == 0) { alreadySet = TRUE; }
    }
    return (alreadySet);
}
...
void SipMessage::setTransaction(SipTransaction *transaction) { mpSipTransaction = transaction; }

SipTransaction *SipMessage::getSipTransaction() const { return (mpSipTransaction); }
...
const UtlString SipMessage::getTransportName(bool &bCustom) const {
    ...
    if (transport.isNull()) {
        ...
        if (transport.isNull()) { bCustom = false; }
    }

    if ("UDP" == transport || "TCP" == transport || "TLS" == transport) {
        bCustom = false;
    } else {
        bCustom = true;
    }
...
void SipMessage::ParseContactFields(const SipMessage *registerResponse,
    ...
    while (registerResponse->getContactEntry(indexContactField, &contactField)) {
        if (strstr(contactField, RequestContactValue) != NULL) {
            ...
            UtlNameValueTokenizer::getSubField(
                contactField.data(), subfieldIndex, ";", &subfieldText);
            while (!subfieldText.isNull()) {
...
#ifdef TEST_PRINT
                osPrintf(
                    "ipMessage::ParseContactFields found contact parameter[%d]: \"%s\" value: "
                    "\"%s\"\n",
                    subfieldIndex,
                    subfieldName.data(),
                    subfieldValue.data());
                ...
                if (subfieldName.compareTo(subField, UtlString::ignoreCase) == 0
                    ...
                    UtlNameValueTokenizer::getSubField(subfieldValue, 1, " \t:;,", &subfieldText);
...
const UtlString &SipMessage::getLocalIp() const { return mLocalIp; }

void SipMessage::setLocalIp(const UtlString &localIp) { mLocalIp = localIp; }

void SipMessage::setTransportInfo(const SipMessage *pMsg) {
    assert(pMsg != NULL);
    if (pMsg) { setLocalIp(pMsg->getLocalIp()); }
}

/// Get the name/value pairs for a Via field
///
///
void SipMessage::parseViaParameters(const char *viaField, UtlContainer &viaParamList)

{
    ...
    do {
        ...
        if (nameAndValuePtr && nameAndValueLength > 0) {
            ...
            while (nameLength > 0 && (*namePtr == ' ' || *namePtr == '\t')) {
                nameLength--;
                namePtr++;
            }

            if (nameLength > 0) {
                int valueSeparatorOffset = strspn(&(namePtr[nameLength]), namValueSeparator);
                const char *valuePtr = &(namePtr[nameLength]) + valueSeparatorOffset;
                int valueLength = nameAndValueLength - (valuePtr - nameAndValuePtr);

                // If there is a value
                if (valueSeparatorOffset <= 0 || *valuePtr == '\0' || valueLength <= 0) {
                    valuePtr = NULL;
                    valueLength = 0;
                }
    ...
    } while (nameAndValuePtr && nameAndValueLength > 0 && viaField[lastCharIndex] != '\0');
}
...
void SipMessage::SipMessageFieldProps::initNames() {
    ...
    mLongFieldNames.insert(
        new NameValuePair(SIP_CONTENT_ENCODING_FIELD, SIP_SHORT_CONTENT_ENCODING_FIELD));
    ...
    mLongFieldNames.insert(
        new NameValuePair(SIP_CONTENT_LENGTH_FIELD, SIP_SHORT_CONTENT_LENGTH_FIELD));
    ...
    NameValuePair *nvPair;
    while ((nvPair = (NameValuePair *)iterator())) {
        mShortFieldNames.insert(new NameValuePair(nvPair->getValue(), nvPair->data()));
    }
}
...
bool SipMessage::smimeEncryptSdp(const void *pEventData) {
    ...
    if (mpSecurity && getSdpBody(mpSecurity) != NULL) {
        ...
        if (pOriginalBody) {
            ...
            SdpBody *pSdpBodyCopy =
                new SdpBody(bodyBytes, bodyLength);// this should be destroyed by
                                                   // the smime destructor,
                                                   // so, we wont destroy it here

            SIPXTACK_SECURITY_ATTRIBUTES *pSecurityAttrib =
...
bool SipMessage::fireSecurityEvent(const void *pEventData,
    ...
    return TapiMgr::getInstance().fireEvent(
        pEventData ? pEventData : mpEventData, EVENT_CATEGORY_SECURITY, &info);
}
...
void SipMessage::normalizeProxyRoutes(const SipUserAgent *sipUA,
    ...
    while (!doneNormalizingRouteSet) {
        ...
        UtlString noValue;
        if (requestUri.getUrlParameter("lr", noValue, 0) && sipUA->isMyHostAlias(requestUri)) {
            /*
